...
int TransmitterIR::setData(RemoteIR::Format format, uint8_t *buf, int bitlength) {
    ...
    switch (format) {
        ...
        case RemoteIR::HELI:
            ticker.detach();
            ticker.attach_us(this, &TransmitterIR::tick, RemoteIR::TUS_HELI);
            break;
...
void TransmitterIR::tick(void) {
    ...
    switch (work.state) {
        ...
        case Leader:
            if (data.format == RemoteIR::NEC) {
            ...
            } else if (data.format == RemoteIR::HELI) {
                /*
                 * HELI.
                 */
                static const int LEADER_HELI_HEAD = 5;
                static const int LEADER_HELI_TAIL = 0;
                if (work.leader < LEADER_HELI_HEAD) {
                    if(work.leader == 0)
                        tx.write(0.33);
                } else {
                    tx.write(0.0);
                ...
                work.leader++;
                if ((LEADER_HELI_HEAD + LEADER_HELI_TAIL) <= work.leader) {
                    work.state = Data;
                }
            } else {
                ;
        ...
        case Data:
            if (data.format == RemoteIR::HELI) {
                ...
                if (!work.data) {
                    ...
                    if(work.data == 1){
                        tx.write(0.33);
                        tmp = data.buffer[work.bitcount / 8] & (1 << (7-work.bitcount % 8));
                    }
                    if (tmp) {
                        if (3 <= work.data) {
                            work.bitcount++;
                ...
                break;
            } else if (data.format == RemoteIR::NEC) {
                /*
                 * NEC.
                 */
                if (work.data == 0) {
        ...
        case Trailer:
            if (data.format == RemoteIR::NEC) {
                 ...
                 * HELI.
                 */
                //static const int TRAILER_HELI_HEAD = 0;
                //static const int TRAILER_HELI_TAIL = 0;
                /*if (work.trailer < TRAILER_HELI_HEAD) {
                    if(!work.trailer)
                        tx.write(0.33);
                } else {*/
                    tx.write(0.0);
                //}
                //work.trailer++;
                //if ((TRAILER_HELI_HEAD + TRAILER_HELI_TAIL) <= work.trailer) {
                    work.state = Idle;
                    ticker.detach();
                //}
            }else{
