...
#define InRange(x,y)   ((((y) * 0.75) < (x)) && ((x) < ((y) * 1.25)))
...
void ReceiverIR::isr_fall(void) {
    ...
    switch (work.state) {
        ...
        case Receiving:
            ...
            } else if (RemoteIR::HELI == data.format) {
                work.d1 = timer.read_us();
            }
            break;
        case Received:
    ...
    switch (work.state) {
        case Idle:
            if (0 <= work.c1) {
                ...
                int a = work.c2 - work.c1;
                if (InRange(a, RemoteIR::TUS_HELI * 5)){
                    data.format = RemoteIR::HELI;
                    work.state = Receiving;
                    data.bitcount = 0;
                } else if (InRange(a, RemoteIR::TUS_SONY * 4)) {
        ...
        case Receiving:
            if (RemoteIR::NEC == data.format) {
                ...
                work.d2 = timer.read_us();
                int a = work.d2 - work.d1;
                if (InRange(a, RemoteIR::TUS_HELI * 3)) {
                 ...
                 */
                timeout.detach();
                timeout.attach_us(this, &ReceiverIR::isr_timeout, RemoteIR::TUS_HELI * 4);
#endif
            }
            break;
        case Received:
